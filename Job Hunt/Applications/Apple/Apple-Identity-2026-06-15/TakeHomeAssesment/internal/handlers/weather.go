...
func GetWeatherHandler(locData <-chan weather.LocationInfo, timeOut time.Duration, locCircLog weather.CircularLog) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			select {
			case <-r.Context().Done():
				// Context canceled the operation
				logger.Warn("Request aborted", slog.Int("HttpStatusCode", http.StatusServiceUnavailable))
				http.Error(w, "Location data retrieval took too long.", http.StatusServiceUnavailable)
				return
			...
			default:
				...
				if !ok {
					...
					logger.Warn("Request timeout", slog.Int("HttpStatusCode", http.StatusServiceUnavailable))
					http.Error(w, "Location data retrieval took too long.", http.StatusServiceUnavailable)
					return
				}
