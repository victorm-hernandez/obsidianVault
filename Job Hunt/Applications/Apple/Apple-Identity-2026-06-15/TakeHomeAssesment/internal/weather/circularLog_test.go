...
func TestLocCircularLog_Log(t *testing.T) {
	logLen := 3
	log := weather.NewLocCircularLog(logLen, time.Minute*5)
	...
	for i := 0; i < logLen*2; i++ {
		addedInfo := weather.LocationInfo{Name: fmt.Sprintf("LocInfo%v", i), ExpiresAt: time.Now().Add(time.Minute * 5)}
		log.Write(addedInfo)
	...
	for i := 0; i < 6; i++ {
		addedInfo := weather.LocationInfo{Name: fmt.Sprintf("LocInfo%v", i), ExpiresAt: time.Now().Add(time.Hour * (-1))}
		log.Write(addedInfo)
	}
	...
	log.Write(weather.LocationInfo{Name: "LocInfoStale1", ExpiresAt: time.Now().Add(time.Minute * (-10))})
	log.Write(weather.LocationInfo{Name: "LocInfoStale2", ExpiresAt: time.Now().Add(time.Minute * (-7))})
	log.Write(weather.LocationInfo{Name: "LocInfoCurrent", ExpiresAt: time.Now().Add(time.Minute * (2))})
...
func TestLocCircularLog_Log_Concurrency(t *testing.T) {
	log := weather.NewLocCircularLog(1000, time.Minute*5)
	...
	for i := 0; i < 20; i++ {
		go func(workerId int) {
			defer wg.Done()
			for j := 0; j < 50; j++ {
				log.Write(weather.LocationInfo{Name: fmt.Sprintf("Loc%v_%v", workerId, j), ExpiresAt: time.Now().Add(time.Minute * 5)})
			}
		}(i)
	}
