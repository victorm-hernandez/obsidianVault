...
type MockLocationFetcher struct {
	Result weather.LocationInfo
	Error  error
}

func (m *MockLocationFetcher) GetLocation(ctx context.Context) (weather.LocationInfo, error) {
	// Simulate delays
...
type MockCircularLog struct {
	data  weather.LocationInfo
	mutex sync.Mutex
}

func (m *MockCircularLog) Write(data weather.LocationInfo) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	m.data = data
}

func (m *MockCircularLog) Read() (weather.LocationInfo, bool) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	return m.data, true
}
...
func TestRandomLocationGenerator_Generate(t *testing.T) {
	testValue := weather.LocationInfo{
		Name:      "Barcelona",
		Longitude: 1.2,
		Latitude:  3.4,
